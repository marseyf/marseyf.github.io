...
resources:
  - assets/source/ct-normal-brain-axial-25.png
format:
  html:
    toc: true
...
![A real axial head CT is shown at five stages of a cosine-scheduled Gaussian diffusion process, from the clean CT through intermediate noisy states to Gaussian noise. Cyan labels mark the known forward corruption process and amber labels mark the idealized learned reverse path.](assets/diffusion-process-ct.png){#fig-diffusion-cycle fig-alt="A real axial head CT at five diffusion timesteps from clean image to Gaussian noise, with arrows identifying forward noising and idealized reverse sampling."}

The clean endpoint is a real CC0 axial CT; every noisy panel uses the same fixed Gaussian field and the forward-process equation below. The reverse arrow describes the learned sampling problem rather than output from a trained model. Visual treatment created with GPT Image 2; CT pixels and diffusion states were composed deterministically. Source CT: [Mikael Häggström, M.D., via Wikimedia Commons](https://commons.wikimedia.org/wiki/File:CT_of_a_normal_brain,_axial_25.png), [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/).
...
::: {.diffusion-video}
<video id="diffusion-cycle-video" width="1280" height="720" controls autoplay muted loop playsinline preload="metadata" poster="assets/diffusion-process-ct.png" aria-label="Forward noising and idealized reverse sampling animation using a real axial CT">
  <source src="assets/diffusion-process-ct.mp4" type="video/mp4" />
</video>

<p><em>Animation:</em> the forward pass applies a known cosine-scheduled Gaussian corruption process. The backward-moving segment is an idealized reverse trajectory: a trained model must estimate the noise at each step.</p>
:::
...
::: {.diffusion-explorer-wrap}
<iframe id="diffusion-explorer" src="assets/timestep-noise-explorer.html" title="Interactive CT diffusion trajectory explorer" width="100%" height="760" loading="lazy"></iframe>

<p><em>Interactive explorer:</em> scrub the same real CT through the forward corruption process or play the idealized reverse trajectory. The CT is source imaging, not generated output; this remains an educational visualization rather than a diagnostic simulation.</p>
:::

<script>
window.addEventListener("message", (event) => {
  if (event.data?.type !== "diffusion-explorer-height") return;
  const explorer = document.getElementById("diffusion-explorer");
  if (explorer && event.source === explorer.contentWindow) {
    explorer.style.height = `${Math.max(680, event.data.height + 2)}px`;
  }
});

const diffusionVideo = document.getElementById("diffusion-cycle-video");
if (diffusionVideo && window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
  diffusionVideo.autoplay = false;
  diffusionVideo.pause();
}
</script>
