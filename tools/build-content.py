...
def publication_list(year_heading=2):
    ...
    for year in sorted({p['year'] for p in papers}, reverse=True):
        ...
        for paper in (p for p in papers if p['year'] == year):
            ...
            entries.append(f'''<article class="publication-entry" id="{e(paper['id'])}"><p class="eyebrow">{e(paper['display_venue'])} · {e(paper['publication_type'])}</p><h{title_heading} class="publication-title">{title}</h{title_heading}><p class="publication-authors">{authors}</p><p class="publication-focus">{e(paper['summary'])}</p>{resource_links(paper)}</article>''')
...
<section class="home-section home-section--tinted home-news" id="news" aria-labelledby="news-title"><div class="section-inner"><h2 id="news-title">News</h2><div class="news-content">{home_news()}</div></div></section>
